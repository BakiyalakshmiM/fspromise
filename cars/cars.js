...
let inve=function(inv){
    let invent = inv.find(invent => invent.id === 33);
    console.log(`Car 33 is a ${invent.car_year} ${invent.car_make} ${invent.car_model}`);
    console.log(`Last car is a ${inv[inv.length-1].car_make} ${inv[inv.length-1].car_model}`);
    inv.sort(function(v1,v2){
        if(v1<v2){
            return -1;
        }
        else{
